...
//
//        // Try to extract the full function text starting from caret
...
//        val funRegex = Regex("""(?:\w+\s+)*fun\s+(?:<[^>]+>\s*)?\w+\s*\([^\)]*\)\s*(?::\s*[^=\{]+)?\s*(?:=|\{)""", RegexOption.DOT_MATCHES_ALL)
...
//        val functionPattern = Regex("""(?:\w+\s+)*fun\s+(?:<[^>]+>\s*)?(\w+)\s*\(([^)]*)\)\s*(?::\s*([^=\{]+))?""", RegexOption.DOT_MATCHES_ALL)
...
//    }
//
//    private fun generateKDoc(name: String, parameters: List<Pair<String, String>>, returnType: String, body: String): String {
...
//        builder.appendLine(" *")
//        for ((p, t) in parameters) {
//            builder.appendLine(" * @param $p TODO: describe $p of type $t")
//        }
//        if (returnType != "Unit") {
//            builder.appendLine(" * @return TODO: return value of type $returnType")
//        }
//
//        val throwRegex = Regex("""throw\s+(\w+)""")
//        val thrown = throwRegex.findAll(body).map { it.groupValues[1] }.toSet()
